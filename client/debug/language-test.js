...
function testLocalStorage() {
  ...
  console.log('Set to Vietnamese:', localStorage.getItem('td_consulting_language'));
  
  localStorage.setItem('td_consulting_language', 'en');
  console.log('Set to English:', localStorage.getItem('td_consulting_language'));
...
function testTranslations() {
  ...
  const reactRootFiber = document.querySelector('#root')._reactInternalFiber ||
...
function testLanguageSwitch() {
  ...
  const languageButton = document.querySelector('[data-testid="language-switcher"]') ||
  ...
  if (languageButton) {
    ...
    setTimeout(() => {
      ...
      if (dropdown) {
        ...
        const viOption = Array.from(dropdown.querySelectorAll('*'))
          .find(el => el.textContent.includes('🇻🇳') || el.textContent.includes('Tiếng Việt'));
        ...
        if (viOption) {
          ...
          setTimeout(() => {
            console.log('New document language:', document.documentElement.lang);
            console.log('New localStorage value:', localStorage.getItem('td_consulting_language'));
          }, 500);
        }
      }
    }, 100);
