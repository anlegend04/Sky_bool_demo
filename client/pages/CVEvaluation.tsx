...
export default function CVEvaluation() {
  ...
  const [selectedSavedEvaluation, setSelectedSavedEvaluation] = useState<CVEvaluationData | null>(null);
  ...
  const handleExportToPDF = async () => {
    ...
    setTimeout(() => {
      ...
      const pdfContent = generatePDFContent(evaluation!, candidateName, jobTitle);
  ...
  const generatePDFContent = (evaluation: CVEvaluation, candidateName: string, jobTitle: string) => {
    return `
CV ANALYSIS REPORT
==================
...
`;
  ...
  const loadSavedEvaluation = (savedEval: CVEvaluationData) => {
    ...
    const candidate = HARDCODED_CANDIDATES.find(c => c.id === savedEval.candidateId);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Dialog open={showSavedEvaluations} onOpenChange={setShowSavedEvaluations}>
        <DialogContent className="sm:max-w-4xl max-h-[80vh]">
            ...
            <div className="space-y-4">
              {savedEvaluations.length === 0 ? (
                <div className="text-center py-8 text-slate-500">
                  ...
                  <p className="text-sm">Complete and save an analysis to see it here</p>
                </div>
              ) : (
                savedEvaluations.map((savedEval) => {
                  ...
                  return (
                    <Card key={savedEval.id} className="hover:shadow-md transition-shadow">
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <h3 className="font-semibold">{candidate?.name || 'Unknown Candidate'}</h3>
                              <Badge variant={savedEval.finalVerdict === "Good Fit" ? "default" :
                                            savedEval.finalVerdict === "Needs Improvement" ? "secondary" : "destructive"}>
                            ...
                            <p className="text-sm text-slate-600 mb-2">
                              <strong>Job:</strong> {job?.position || 'Unknown Position'}
                            </p>
                            <p className="text-sm text-slate-600 mb-2">
                              <strong>File:</strong> {savedEval.fileName}
                            </p>
                            <p className="text-xs text-slate-500">
                              Saved on {new Date(savedEval.savedAt).toLocaleDateString()} by {savedEval.createdBy}
                            </p>
                            {savedEval.isShared && (
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            {savedEval.shareUrl && (
                              <Button
                                ...
                                variant="outline"
                                onClick={() => {
                                  navigator.clipboard.writeText(savedEval.shareUrl!);
                                  toast({
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowSavedEvaluations(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
