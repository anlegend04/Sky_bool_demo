...
const suppressRechartsWarnings = (...args: any[]): boolean => {
  ...
  const isRechartsComponent = /\b(XAxis|YAxis|XAxis2|YAxis2|CartesianGrid|Tooltip|Legend|ResponsiveContainer|BarChart|LineChart|PieChart|Area|Bar|Line|Pie|Cell|Surface|ReferenceLine|ReferenceArea|Brush|FunnelChart|Funnel|RadarChart|Radar|PolarGrid|PolarAngleAxis|PolarRadiusAxis|ChartLayoutContextProvider|CategoricalChartWrapper)\b/.test(message);
...
if (typeof window !== 'undefined') {
  ...
  const interceptReactWarnings = () => {
    ...
    if (reactDevTools && reactDevTools.onCommitFiberRoot) {
      ...
      reactDevTools.onCommitFiberRoot = (id: any, root: any, ...args: any[]) => {
        try {
    ...
    if (anyWindow.React && anyWindow.React.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED) {
      ...
      if (internals.ReactDebugCurrentFrame && internals.ReactDebugCurrentFrame.getCurrentStack) {
